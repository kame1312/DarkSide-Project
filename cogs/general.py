...
class General(commands.Cog, name="general"):
    ...
    async def help(self, context: Context) -> None:
        ...
        for cog_name, cog in self.bot.cogs.items():
            ...
            help_text = "\n".join(data)
            embed.add_field(name=cog_name.replace("_", " ").title(), value=f"```{help_text}```", inline=False)
    ...
    async def serverinfo(self, context: Context) -> None:
        ...
        embed = discord.Embed(title="**Server Name:**", description=f"{context.guild}", color=COLOR_DEFAULT)
        ...
        embed.add_field(name="Created at", value=discord.utils.format_dt(context.guild.created_at, style="F"), inline=False)
    ...
    async def ping(self, context: Context) -> None:
        embed = discord.Embed(title="🏓 Pong!", description=f"The bot latency is {round(self.bot.latency * 1000)}ms.", color=COLOR_DEFAULT)
        await context.send(embed=embed)

    @commands.hybrid_command(name="invite", description="Get the invite link of the bot to be able to invite it.")
    async def invite(self, context: Context) -> None:
        embed = discord.Embed(description=f"Invite me by clicking [here]({self.bot.invite_link}).", color=COLOR_ACCENT)
        try:
